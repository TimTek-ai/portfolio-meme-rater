...
export function TickerInput({ onSubmit }: TickerInputProps) {
  const [entries, setEntries] = useState<TickerEntry[]>([
    { ticker: "", shares: "", purchasePrice: "", currentPrice: null, loading: false, error: null },
  ]);
  ...
  const fetchPrice = async (index: number, ticker: string) => {
    ...
    try {
      ...
      setEntries((prev) =>
        prev.map((e, i) =>
          i === index
            ? { ...e, currentPrice: data.price, loading: false, ticker: ticker.toUpperCase() }
            : e
        )
      );
    } catch (error) {
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4 space-y-4">
      ...
      <p className="text-sm text-gray-400">
        Enter ticker symbols to auto-fetch current prices
      </p>
      ...
      <div className="flex gap-2">
        <button
          onClick={addEntry}
          className="flex-1 py-2 px-4 bg-gray-700 hover:bg-gray-600 rounded text-sm transition-colors"
        >
          + Add Ticker
        </button>
        <button
          ...
          disabled={!allValid}
          className="flex-1 py-2 px-4 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 disabled:cursor-not-allowed rounded text-sm font-medium transition-colors"
        >
          Generate Memes
        </button>
      </div>
